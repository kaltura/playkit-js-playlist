...
export const preparePage = (puginConf = {}, playbackConf = {}): PromiseLike<any> => {
  return cy.visit('index.html').then($win => {
    try {
      ...
      // @ts-ignore
      return $win.kalturaPlayer.loadPlaylist({playlistId: '0_wifqaipd'}).then(() => $win.kalturaPlayer);
    } catch (e: any) {
      Cypress.log({
...
export const mockKalturaBe = (playlistFixture = 'playlist.json', userEntryFixture = 'user_entry.json') => {
  cy.intercept('http://mock-api/service/multirequest', req => {
    ...
    if (checkRequest(req.body[2], 'userEntry', 'list')) {
      return req.reply({fixture: userEntryFixture});
    }
  });
};
